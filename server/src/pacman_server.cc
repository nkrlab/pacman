// Copyright (C) 2012 Nexon Korea Corporation All Rights Reserved.
//
// This work is confidential and proprietary to Nexon Korea Corporation and
// must not be used, disclosed, copied, or distributed without the prior
// consent of Nexon Korea Corporation.

#include <boost/bind.hpp>
#include <funapi/account/account_event_handler_registry.h>
#include <funapi/account/object_creator_registry.h>
#include <funapi/api/tick/world_event_handler_registry.h>
#include <funapi/framework/installer.h>

#include "funapi_event_handlers.h"

namespace {

  class PacmanServerInstaller : public fun::framework::Installer {
  public:
    virtual bool Install(
        const fun::framework::Installer::ArgumentMap &/*arguments*/) {
      fun::ObjectCreatorRegistry::Install(
          boost::bind(pacman::CreateObject,
                      pacman::kAccountObjectModelName),
          pacman::DeserializeObject);

      fun::WorldEventHandlerRegistry::Install(
          pacman::OnWorldReady,
          std::make_pair(pacman::OnWorldTick,
                         pacman::kWorldTickMicrosecond));

      fun::AccountEventHandlerRegistry::Install(
          pacman::OnAccountLogin,
          pacman::OnAccountLogout,
          pacman::OnAccountTimeout,
          pacman::OnAccountMessage);

      return true;
    }

    virtual bool Uninstall() {
      return true;
    }
  };

}  // unnamed namespace

REGISTER_INSTALLER(PacmanServer, PacmanServerInstaller)
