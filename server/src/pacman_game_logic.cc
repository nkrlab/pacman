...
// internal value for rand_r() func
unsigned int rand_internal = 0;
...
// pacman_event_world.cc에 선언된 World 변수를 쓰기 위해 선언한다.
// 이 변수를 통해 world라는 fun object에 접근할 수 있다.
extern PacmanPtr the_world;


// Attribute에 포함되지 않는 변수를 초기화한다.
// Level이 Load 될 때 한번 호출된다.
...
void CheckCollision() {
  // 캐릭터들의 시작 위치를 가져온다.
  const std::string &start_pos = the_world->start_points();
  // 캐릭터들의 life(살아있는지 죽어있는지)를 얻어온다.
  std::string str_characters_lives = the_world->characters_lives();
  ...
  for (int i = 0; i < (kChrIndexGhost3+1); ++i) {
    // 캐릭터들의 위치값을 얻어온다.
    std::string locations = the_world->locations();
    ...
    if ((locations[index_x] == locations[pacman_x]) &&
      ...
      if (the_world->invincible() == 1) {  // 무적상태이면
        // Ghost가 죽어야 한다
        // game point를 계산하여 값을 지정한다.
        // 이 값은 자동으로 클라이언트에 전송된다.
        the_world->set_game_points(the_world->game_points() +
            (the_world->ghosts_in_a_row() * 20));
        ...
        // 무적이 풀리면 다시 초기화 된다.
        // 이 값은 자동으로 클라이언트에 전송된다.
        the_world->set_ghosts_in_a_row(the_world->ghosts_in_a_row() * 2);
        ...
        the_world->set_locations(locations);
      } else {  // 무적상태가 아닌 상태에서 충돌이면 팩맨이 죽은것.
        // 남아있는 life를 차감한다.
        int remain_lives = the_world->remain_lives();
        --remain_lives;
        // 계산된 life 값을 지정한다.
        // 이 값은 자동으로 클라이언트로 전송된다.
        the_world->set_remain_lives(remain_lives);

        // 남아있는 life가 없다면 Game Over
        if (remain_lives == -1) {
          // exit message를 지정하며 이 값은 자동으로 클라이언트에 전송된다.
          the_world->set_exit_message("Game Over");
        }
...
void MoveGhosts() {
  const int invincible = the_world->invincible();
  const std::string &cells = the_world->level();

  if (invincible == 1) {
    ++slower_ghosts;
    if (slower_ghosts > kHowSlow) {
      slower_ghosts = 0;
    }
  }

  if ((invincible == 0) || slower_ghosts < kHowSlow) {
...
void MovePacman() {
  const std::string &directions = the_world->directions();

  std::string locations = the_world->locations();
  std::string cells = the_world->level();
  ...
  switch (cells[index_cell]) {
    case kLevelCellPellet: {   // Pellet
      cells[index_cell] = 0;
      the_world->set_level(cells);

      int game_points = the_world->game_points();
      ++game_points;
      the_world->set_game_points(game_points);

      int left_pellets = the_world->left_pellets();
      --left_pellets;
      if (left_pellets <= 0) {
        the_world->set_level_number(the_world->level_number() + 1);
      } else {
    ...
    case kLevelCellPowerUp: {   // PowerUp
      cells[index_cell] = 0;
      the_world->set_level(cells);
      the_world->set_invincible(1);
      if (the_world->ghosts_in_a_row() == 0)
        the_world->set_ghosts_in_a_row(1);
        invincible_time = time(0);
    }
    break;
  }
  ...
  // Is he invincible?
  int time_left = 0;
  if (the_world->invincible() == 1) {
    time_left = (11 - the_world->level_number() - time(0) + invincible_time);
  }
  ...
  if (time_left < 0) {
    ...
    the_world->set_time_left(time_left);
  }
}
