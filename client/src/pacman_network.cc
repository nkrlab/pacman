...
boost::uuids::uuid global_uu = { { 0U, 0U, 0U, 0U,
                                   0U, 0U, 0U, 0U,
                                   0U, 0U, 0U, 0U,
                                   0U, 0U, 0U, 0U } };
...
void PackStringToBuffer(const std::string &strarg,
  ...
  *arg_buffer = buffer;
  ...
  ::memcpy(arg_buffer->get() + length_size, global_uu.data, uuid_size);
  ::memcpy(arg_buffer->get() + length_size + uuid_size, strarg.c_str(),
           strarg.size());
}


bool GetAttributeInt(
    const AttributeUpdatesMessage_AttributeUpdateMessage &attribute_msg,
    const char *kName,
    int *return_value) {
  const std::string &attribute_name = attribute_msg.attribute_name();
  if (attribute_name.compare(kName) != 0)
    return false;

  json_spirit::Value value;
  json_spirit::read(attribute_msg.new_json(), value);
  *return_value = value.get_int();

  return true;
}


bool GetAttributeString(
    const AttributeUpdatesMessage_AttributeUpdateMessage &attribute_msg,
    const char *kName,
  ...
  json_spirit::Value value;
  json_spirit::read(attribute_msg.new_json(), value);
  *return_value = value.get_str();

  return true;
}


void HandlingReceivedPacket(const std::string &buffer) {
  ...
  if (account_type == ServerAccountMessage::kAttributeUpdatesMessage) {
    ...
    for (int i = 0; i < updates_size; ++i) {
      const AttributeUpdatesMessage_AttributeUpdateMessage &attrib_msg =
          updates.attribute_update(i);

      if (attrib_msg.has_attribute_name()) {
        int int_value = 0;
        std::string str_value = "";
        if (GetAttributeInt(attrib_msg, "game_points", &int_value)) {
          OnChangeGamePoints(int_value);
        } else if (GetAttributeInt(attrib_msg, "ghosts_in_a_row", &int_value)) {
          OnChangeGhostsInARow(int_value);
        } else if (GetAttributeInt(attrib_msg, "invincible", &int_value)) {
          OnChangeInvincible(int_value);
        } else if (GetAttributeInt(attrib_msg, "left_pellets", &int_value)) {
          OnChangeLeftPellets(int_value);
        } else if (GetAttributeInt(attrib_msg, "level_number", &int_value)) {
          OnChangeLevelNumber(int_value);
        } else if (GetAttributeInt(attrib_msg, "rand_internal", &int_value)) {
          OnChangeRandInternal(int_value);
        } else if (GetAttributeInt(attrib_msg, "remain_lives", &int_value)) {
          OnChangeRemainLives(int_value);
        } else if (GetAttributeInt(attrib_msg, "time_left", &int_value)) {
          OnChangeTimeLeft(int_value);
        } else if (GetAttributeString(attrib_msg, "locations", &str_value )) {
          OnChangeLocations(str_value);
        } else if (GetAttributeString(attrib_msg, "level", &str_value )) {
          OnChangeLevel(str_value);
        } else if (GetAttributeString(attrib_msg, "characters_lives",
                   &str_value)) {
          OnChangeCharactersLives(str_value);
        } else if (GetAttributeString(attrib_msg, "exit_message",
                                      &str_value)) {
          OnChangeExitMessage(str_value);
        } else {
          // do nothing
        }
      }
 ...
 private:
  ...
  void UnpackString() {
    while (current_buffer_index_ > sizeof(uint32_t)) {
      ...
      boost::uuids::uuid uu;
      ::memcpy(uu.data, read_buffer_ + sizeof(uint32_t),
